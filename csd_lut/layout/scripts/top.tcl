...
set init_verilog "../syn/results/csd_lut.mapped.v"
...
saveDesign db/csd_lut_floorplan.enc
...
saveDesign db/csd_lut_powerplan.enc
...
saveDesign db/csd_lut_place.enc
...
setNanoRouteMode -quiet -routeWithSiDriven trueesign -globalDetail
...
saveDesign db/csd_lut_postroute.enc
...
verifyGeometry -report reports/csd_lut.geometry.rpt
verifyConnectivity -type all -report reports/csd_lut.connectivity.rpt
...
report_area -out_file reports/csd_lut.area.rpt

# Power report
report_power -outfile reports/csd_lut.power.rpt

# Setup Timing Report
report_timing -machine_readable -max_paths 100 -max_slack 100.0 -min_slack -100.0 -late > csd_lut_setup.mtarpt

# Hold Timing Report
report_timing -machine_readable -max_paths 100 -max_slack 100.0 -min_slack -100.0 -early > csd_lut_hold.mtarpt

# ------------------------------------------------------------------------------
# Step 7b. Exports the design
# ------------------------------------------------------------------------------
# SPEF file: parastic data of wires
rcOut -spef results/csd_lut.spef -rc_corner rc_worst

# SDF file: delay files for gates and wires
write_sdf results/csd_lut.sdf

# *.v: verilog netlist file
saveNetlist results/csd_lut.v

# *.gds: GDS file
streamOut results/csd_lut.gds -mapFile streamOut.map \
